...
class NotificationList:
    ...
    @staticmethod
    async def notify_users_about_x(trackers: list, param_1: str, param_2: str):
        """
        ...
        for user in trackers:
            asyncio.create_task(
                NotificationFuncs.send_notification(notifications_queue, message_0, message_1, message_2, msg=msg))
            await notifications_queue.put(user)
        await notifications_queue.join()

    @staticmethod
    async def notify_users_about_y(trackers: list, param_1: str, param_2: str):
        """
        formats the notification pattern and notification details before sending to users in trackers list
        ...
        for user in trackers:
            asyncio.create_task(NotificationFuncs.send_notification(notifications_queue, message, msg=msg))
            await notifications_queue.put(user)
        await notifications_queue.join()

    @staticmethod
    async def notify_users_about_z(trackers: list, *args: tuple):
        """
        formats the notification pattern and notification details before sending to users in trackers list
